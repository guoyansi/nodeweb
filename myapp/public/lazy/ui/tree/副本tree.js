(function($){
    ...
    var getNode=function (d,obj) {
        var attrs={};
        for(var k in d){
            attrs[k]=obj.attr("tree-"+k);
        }
        return attrs;
    };
	var defaults={
	...
	$.fn.ui_tree=function(opt){
		...
		objBox.on("click",".ui-tree-title",function(){
			...
			obj.addClass("active");
			resetTitleW(objBox);
			var attrs=getNode(opt.data[0],$(this));
			objInput.val(attrs.text);
			objBox.hide();
			opt.click(attrs);
			...
			if(obj.is(".ui-tree-icon-hide")){//开始 show
				...
				if(childDom.length>0){//有子节点dom
					...
					var node=getNode(opt.data[0],objTitle);
					opt.getChild(node,function(data){
